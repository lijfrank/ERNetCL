...
def get_DailyDialogue_loaders(path='', batch_size=32, num_workers=0, pin_memory=False):
    ...
    trainset = DailyDialogueRobertaDataset(path=path, split='train')
    validset = DailyDialogueRobertaDataset(path=path, split='valid')
    testset = DailyDialogueRobertaDataset(path=path, split='test')
...
def train_or_eval_model(model, loss_function, loss_function_cl, dataloader, epoch, optimizer=None, train=False, cuda_flag=False, loss_type='',lambd=0.0,epochs=100):
    ...
    avg_macro = round(f1_score(labels,preds, sample_weight=masks, average='macro')*100, 2)
    avg_micro = round(f1_score(labels,preds, sample_weight=masks, average='micro', labels=[0,2,3,4,5,6])*100, 2)
    fscores = [avg_macro, avg_micro]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ran_mode', default='sran1', help='sran1')
    ...
    parser.add_argument('--norm_first', action='store_true', default=False, help='norm_first')
    parser.add_argument('--use_residual', action='store_true', default=True, help='use residual connection')
    parser.add_argument('--use_speaker', action='store_true', default=False, help='use speaker connection')
    parser.add_argument('--rnn_model', default='GRU')
    parser.add_argument('--rnn_layer', type=int, default=3, help='')
    ...
    parser.add_argument('--sigma', type=float, default=0.6, help='cl_loss')
    parser.add_argument('--delt_epoch', type=int, default=9, help='cl_loss')
    ...
    if args.class_weight:
        ...
        loss_function_cl = CurrLearnLoss(loss_weights, delt_epoch=args.delt_epoch, sigma=args.sigma,dataset='DailyDialog')
    else:
        if args.mu > 0:
            loss_weights = torch.FloatTensor(create_class_weight(args.mu))
        else:   
    ...
    lf = open('logs/dailydialog_logs.txt', 'a')
    lf.write(str(args) + '\n')
    ...
    for e in range(n_epochs):
        ...
        if (e+1)%10 == 0:
            ...
            x4 = 'test_best_acc: {}, [test_best_macro, test_best_micro w/o Neutral]: {}'.format(best_acc, best_fscore)
            print(x4)
            lf.write(x4 + '\n')
