...
def get_IEMOCAP_loaders(path='', batch_size=32, num_workers=0, pin_memory=False):
    trainset = IEMOCAPRobertaDataset(path=path, split='train')
    validset = IEMOCAPRobertaDataset(path=path, split='valid')
    testset = IEMOCAPRobertaDataset(path=path, split='test')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rnn_model', default='GRU', help='GRU\LSTM')
    parser.add_argument('--rnn_layer', type=int, default=2, help='')
    ...
    parser.add_argument('--sigma', type=float, default=0.7, help='any')
    parser.add_argument('--delt_epoch', type=int, default=9, help='control mu, need >1')
    ...
    if args.class_weight:
        ...
        loss_function_cl = CurrLearnLoss(loss_weights, delt_epoch=args.delt_epoch, sigma=args.sigma,dataset='IEMOCAP')
    else:
        loss_weights = torch.FloatTensor([1/0.086747,
                                        1/0.144406,
                                        1/0.227883,
